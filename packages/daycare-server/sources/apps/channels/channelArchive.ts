...
export async function channelArchive(
  ...
  const channel = await databaseTransactionRun(context.db, async (tx) => {
    ...
    if (channel.archivedAt) {
      throw new ApiError(400, "VALIDATION_ERROR", "Channel is already archived");
    }
    ...
    if (!actorMembership || actorMembership.role !== "OWNER") {
      throw new ApiError(403, "FORBIDDEN", "Only channel owners can archive channels");
    }
